...
The skill runs **`codex exec` with `-s workspace-write`** — Codex operates with write access
to your workspace. That is why it is not bundled with the extension; installing it is meant
to be a deliberate act.

- In the default modes (consult / review) Codex is instructed to write **exactly one file**,
  the response document, and any other change is **measured against the filesystem and
  reported** (see §6).
...
**You can keep working while it runs.** It usually takes 1–3 minutes and nothing blocks.
...
> 💡 **Smaller requests are dramatically faster.** Measured on the same codebase: asking Codex
...
Codex actually said, and because the files are committed the next session picks up the thread.
...
Each run adds roughly **300 KB**, about 86% of which is captured command output.
...
- **Automatic** — enable `codexRunAutoCleanup` in settings and the extension cleans up once
  per activation (default: keep 7 days). Live or still-locked runs are never touched
...
- **unresponsive** means the process looks killed. It is never promoted to "done".
- A **completion chime** plays when a run finishes (setting: `claudeContextBar.workflowCompleteBeep`).
...
| Fix mode (Codex editing directly) | ✅ measured — but only a **single-line fix**; multi-file changes untested |
| Progress panel · completion chime · live updates | ✅ measured |
| "review against main" / "just this commit" | ❌ dry-run only |
| Running on a Linux server | ❌ install and syntax only |
| Progress panel over Remote-SSH | ❌ unsupported (the extension reads local files only) |
...
| The answer is identical to last time | Codex wrote nothing this run — it's flagged as `stale` |
...
| `CR_DRYRUN=1` | Print the assembled command without running it |
