...
- **Task (Agent tool) sub‑agents** — sub‑agents spawned via Claude Code's Agent tool are shown as their own cards, grouped into **batches by start time** (5‑minute gap = new batch). They are handled the same way as workflows: 🗑 on a finished batch sends it to the same trash, and a sub‑agent the conversation reports as stopped, killed or failed is shown as stopped. If it has written more since that report, it was resumed and stays running
- **Trash** — 🗑 on a finished workflow or batch moves it aside instead of destroying it, with no confirmation to click through. Open 🗑 at the top of the panel to restore it or delete it for good; the trash covers every session of the project. Restoring is refused if a live workflow has since taken that id, or a sub‑agent log of the same name is back in place
...
- **Orange** — a workflow in one of the status bar's sessions has an agent running, or an Agent-tool sub-agent is running there. Sub-agents launched together count as one batch, as in the workflow panel
...
## 🤝 peer_req — open sessions asking each other (plugin)

A separate Claude Code plugin from this repository, not part of the extension. It lets your open sessions ask and notify each other directly — two repositories on the same PC, your PC and a server, or two servers — with a request ID, an acknowledgement and a record on both sides, so nothing is confused between "sent" and "read" and nothing runs twice. When the other session isn't open, it can run the request unattended in that repository after asking you.

**It requires Remote Control to be on in every session you pair.** Sessions on other machines are reachable only through it. Install with `/plugin marketplace add comonetso/claudeStateBar` and `/plugin install peer-req@comonetso`; setup, the address book and the Remote Control steps are in the [peer_req README](skills/peer_req/README.md) ([한국어](skills/peer_req/README.ko.md)). The extension's auto-update notice (see codex_rescue above) covers this plugin too.

## 🔍 browser_check — Claude checks your real browser (plugin)

A third plugin from this repository, also separate from the extension. Instead of reporting "tests pass" from server logs and source alone, Claude Code opens your real browser (Aside, which is Chromium-based) and checks the page itself: the screen, console errors, failed requests, CSS, responsive and dark layouts, accessibility and performance. It runs from a server session driving the browser on your PC, or locally on the PC, and starts from plain Korean requests such as "화면 확인해 봐" (check the screen) or "콘솔 에러 봐" (look at console errors).

A policy layer sits between Claude and the browser. It works only in tabs it opened itself, changes data only with your approval for that exact change, and does not read or store secrets. Driving the browser on another machine needs either Aside Pro's remote host or an SSH tunnel from the PC to the server. Install with `/plugin install browser-check@comonetso`. On first use it looks the machine over, writes its private settings file and shows you what it put in; the tunnel and the full safety rules are in the [browser_check README](skills/browser_check/README.md) ([한국어](skills/browser_check/README.ko.md)). This is an early version, measured on a Windows PC and over a real PC → server tunnel. The extension's auto-update notice covers this plugin too.
