...
A policy layer sits between Claude and the browser. It works only in tabs it opened itself, changes data only with your approval for that exact change, and does not read or store secrets. Driving the browser on another machine needs either Aside Pro's remote host or an SSH tunnel from the PC to the server. Install with `/plugin install browser-check@comonetso`; the private settings file, the tunnel and the full safety rules are in the [browser_check README](skills/browser_check/README.md) ([한국어](skills/browser_check/README.ko.md)). This is an early version (0.1.0): running entirely on the PC and a real SSH tunnel have not been measured yet. The extension's auto-update notice covers this plugin too.
