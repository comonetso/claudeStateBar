...
> Time spent, lines changed, tokens used and what they would cost at API rates, in a panel beside
> your code.
...
actual work, lines changed, requests made, and tokens used — with what those tokens would have cost
at Anthropic's published API rates. It is an estimate, not your bill.
...
in, each group counting how many of its agents have completed, and every row shows the model that ran
it and the tokens it used. Click a phase to fold it; phases with no running agents start folded, and what
you fold is remembered after the panel is closed. Where a workflow script named its agents, those
names are used instead of a guess from the prompt. Elapsed times read in Korean when the panel is in Korean.

### Codex runs

Each run card now shows the model and reasoning effort Codex actually ran with, next to the stamp
and thread id. After a follow-up, it shows the latest turn's.

### Fixes

- Opus 5 sessions could show a context percentage up to five times too high. Some older Sonnet
  models were held to the wrong limit too.
