...
그래서 창이 닫힌 신호는 **움직이던 `resetsAt` 이 멈추는 것**이지 사용률이 0%가 되는 게 아닙니다. 사용률로는 판단할 수 없습니다 — 열려 있는 창도 안 쓰면 0%라서, 그렇게 읽으면 절전에서 깰 때마다 리셋됐다고 알립니다. 지금은 진짜 열림 → 닫힘 전환에서만 발사합니다.

### 과금 안전장치

Codex가 API 키로 로그인되어 있으면 `codex exec` 는 5시간 창이 아니라 API 크레딧으로 나가고, 어느 쪽이든 종료 코드는 0입니다. 그래서 프라이머는 발사 전에 `auth.json` 을 읽고, ChatGPT 플랜 로그인(`auth_mode: "chatgpt"`)이면서 `OPENAI_API_KEY` 가 파일에도 환경에도 없을 때만 쏩니다. `auth.json` 을 못 읽으면 그것도 거부로 칩니다 — 돈이 나가는 문제를 추측으로 넘기지 않으려고 둔 검사입니다. 거부하면 설정을 끄고 이유를 알려줍니다.
