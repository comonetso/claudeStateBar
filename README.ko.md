...
5시간 블록은 리셋 시각이 아니라 **첫 메시지 시각**에 앵커됩니다. 그래서 04:00에 리셋되어도 08:00에 입력을 시작하면 블록은 08:00~13:00이 됩니다.

**`claudeState.autoStartBlockOnReset`** 를 켜면, **텔레그램 리셋 알림을 쏘는 바로 그 지점에서** 더미 `claude -p` 프롬프트를 함께 발사해 새 블록을 리셋 시각에 앵커합니다 (04:00 리셋 → 깔끔한 04:00~09:00 블록). 발사는 전용 임시 디렉토리에서 이뤄지며, 이 더미 세션은 상태바에서 자동으로 걸러집니다.

⚠️ **트레이드오프를 알고 켜세요:** 새 창은 **즉시 카운트다운을 시작합니다 — 자는 동안에도요.** 이 기능은 블록 경계를 리셋 시각에 고정하려는 용도이지, "일어났을 때 새 블록이 기다리게" 하려는 용도가 **아닙니다** (오히려 정반대 효과입니다). 그 목적이 아니라면 꺼두세요.

`claude` CLI가 PATH에 있어야 하고, VS Code가 켜져 있어야 합니다. VS Code 창이 여러 개 열려 있어도 원자적 락 파일로 **한 창만** 발사합니다. 이미 블록이 열려 있는 상태라면 발사를 건너뜁니다 — 앵커할 것이 없기 때문입니다.

### 과금 안전장치 — 검증하고, 스스로 꺼집니다

이 기능은 headless `claude -p` 실행이 **구독**으로 처리될 때만 의미가 있습니다. Anthropic이 이를 **API 과금**으로 돌리겠다고 예고한 적이 있고, 만약 그렇게 되면 호출은 여전히 exit 0으로 **성공**하는데 5시간 창은 열리지 않습니다 — 조용한 실패가 아니라 **조용한 과금**입니다. 그래서 exit 코드를 절대 신뢰하지 않습니다:

- **발사 거부** — 환경에 `ANTHROPIC_API_KEY` / `ANTHROPIC_AUTH_TOKEN` 이 있으면 아예 쏘지 않습니다 (그 호출은 구독이 아니라 API 크레딧으로 나갑니다).
- **사후 검증** — 발사 후 claude.ai 사용량을 다시 읽어 5시간 창이 **실제로 미래로 이동했는지** 확인합니다. 구독 블록이 열렸다는 증거는 exit 코드가 아니라 이것입니다.
- **자동 정지** — 검증에 실패하면 설정을 **즉시 OFF** 로 바꾸고 텔레그램·VS Code로 경고합니다. 매 리셋마다 조용히 반복되지 않습니다. 최악의 경우 손실은 **딱 1회분**입니다.
