...
이걸 걸면 **CONSULT 첫 턴**이 app-server 배관으로 돌면서 끼어들기를 받습니다. 나머지는 그대로입니다 — 요청서 검증·lock·변경 감지·EDIT 게이트·응답 회수·되묻기 경로가 전부 원래 자리에 있습니다. 되묻기·리뷰·수정은 각자 검증된 배관을 그대로 쓰며, 한꺼번에 갈아타면 회귀 범위가 통째로 열리기 때문에 일부러 좁혔습니다.

배관은 스킬의 `scripts/` 에 함께 들어 있습니다. 설치할 때 `send.sh` 와 같이 받으시면 됩니다([설치 안내](docs/codex-rescue-guide.ko.md#0-설치)에 포함돼 있습니다). 없으면 조용히 옛 경로로 떨어지지 않고 **거부**하므로, 어느 경로로 도는지 항상 알 수 있습니다.
