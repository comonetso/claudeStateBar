...
이걸 걸면 **분석(CONSULT)·수정(EDIT)·리뷰(REVIEW)의 첫 턴**이 끼어들기를 받는 경로로 돕니다. 처음에는 분석 첫 턴에만 걸었다가, 스킬 쪽에서 수정과 리뷰까지 넓혔습니다. 요청서 검증·lock·변경 감지·수정 승인 게이트·응답 회수는 원래 자리에 그대로 있습니다. 되묻기와 핑퐁은 대화를 이어받는 경로라 여전히 끼어들 수 없습니다.
...
Codex의 **primary**(5시간) 창도 같은 앵커 모델이라 사정이 똑같습니다. 자리를 비운 사이 창이 닫히면 다음에 Codex를 돌리기 전까지 아무것도 열리지 않습니다. 확장은 상태바 표시를 위해 이미 Codex 계정 한도를 폴링하고 있고, 이제 그 값에서 창이 닫히는 순간을 잡아냅니다.

**5시간 한도가 실제로 있는 계정만 해당됩니다.** Plus 에는 있고 Pro 는 현재 주간 한도만 있습니다. 플랜 이름을 보는 게 아니라 **5시간 창이 실제로 오는지**로 판단합니다 — OpenAI 가 Pro 에도 도입한다고 했고, 이렇게 해두면 그날 코드를 고칠 필요가 없습니다. 한도가 없는 계정은 리셋 알림과 자동 시작에서 아예 제외되고, 상태바에는 빈칸 대신 **주간 사용률**이 표시됩니다.
