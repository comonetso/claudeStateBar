...
그래서 창이 닫힌 신호는 **고정돼 있던 `resetsAt` 이 움직이기 시작하는 것**이지 사용률이 0%가 되는 게 아닙니다. 사용률로는 판단할 수 없습니다 — 열려 있는 창도 안 쓰면 0%라서, 그렇게 읽으면 절전에서 깰 때마다 리셋됐다고 알립니다. 지금은 진짜 열림 → 닫힘 전환에서만 발사합니다. 열려 있는 창의 시각도 폴링 사이에 1초쯤 흔들리기 때문에, "고정"은 5초까지 허용합니다 — 닫힌 창은 최소 65초씩 움직이므로 그 사이에서 갈립니다.

창이 닫히는 순간은 한 번뿐이라, 그 순간을 놓치면 — 그때 설정이 꺼져 있었거나, 프라이머가 돌았는데 실패했거나 — 저절로 다시 오지 않습니다. 그래서 **10분** 넘게 닫힌 채로 있으면 전환을 기다리지 않고 창을 하나 엽니다. 닫혀 있는 동안 같은 10분 간격으로 다시 시도합니다. 이 복구는 텔레그램을 보내지 않습니다 — 리셋이 아니고, 프라이머가 계속 실패하면 같은 알림이 10분마다 반복될 것이기 때문입니다.
