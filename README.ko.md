...
- **주황** — 상태바에 보이는 세션에서 에이전트가 실행 중인 워크플로우가 있을 때. Agent 도구 서브에이전트는 켜지 않습니다
...
- **마우스 올리기·누르기** — 툴팁에 무엇이 도는지가 나옵니다. 워크플로우마다 이름과 에이전트가 몇 개 끝났는지, 명령마다 설명과 시작 시각, Codex 실행마다 제목과 모드입니다. 점을 누르면 그 패널이 열립니다
