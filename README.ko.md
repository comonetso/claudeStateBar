...
**지금은 배관을 직접 불러서 씁니다.** 위 설명은 전부 [`tools/live-consult/`](tools/live-consult/)로 실제 돌린 결과입니다 — `run`으로 자문을 시작하고, 도는 중에 `steer`로 한 줄 던지면 됩니다.

아직 안 된 것은 **지름길**입니다. `send.sh`가 아직 `codex exec` 경로를 타므로, 평소처럼 "분석해줘"라고 하면 끼어들기 없는 옛 경로로 갑니다. 둘을 잇는 것이 다음 판이며 남은 결정 사항까지 [통합 계획](docs/LIVE_STEER_INTEGRATION.md)에 적어 두었습니다.
