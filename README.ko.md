...
- **단계별로 묶어서 보여줌** — 워크플로우가 단계를 둘 이상 선언했으면 에이전트를 그 단계 아래로 묶고, 묶음마다 완료/전체 숫자를 달아 보여줍니다. 어느 에이전트가 어느 단계에 속했는지는 디스크 어디에도 기록되지 않아 워크플로우 대본을 읽어 되짚습니다. 배정하지 못한 에이전트는 예전처럼 그냥 나열되고, 배정된 것이 너무 적으면 카드 전체가 평범한 목록으로 돌아갑니다
...
이걸 걸면 **CONSULT 첫 턴**이 끼어들기를 받는 경로로 돕니다. 나머지는 그대로입니다 — 요청서 검증·lock·변경 감지·EDIT 게이트·응답 회수·되묻기 경로가 전부 원래 자리에 있습니다. 되묻기·리뷰·수정은 각자 검증된 배관을 그대로 쓰며, 한꺼번에 갈아타면 회귀 범위가 통째로 열리기 때문에 일부러 좁혔습니다.
...
여기에 금액도 붙습니다. **비용 (API 환산)** 은 그 대화의 토큰을 Anthropic 공식 API 단가로 환산합니다 — 100만 토큰당 입력/출력 기준으로 Opus $5/$25, Sonnet $2/$10, Haiku $1/$5, Fable $10/$50 이고, 캐시 쓰기는 입력의 1.25배, 캐시 읽기는 0.1배입니다(Fable 만 캐시 읽기가 $0.25 고정). 이 대화를 API 로 돌렸다면 얼마였을지가 바로 보입니다. 구독이라 실제로 청구되는 금액은 없고, 그래서 Claude Code 자신은 같은 세션을 `$0` 으로 기록합니다. 금액에 마우스를 올리면 단가와 모델별 내역이 나옵니다. 공식 단가가 없는 모델은 추측하지 않고 합계에서 빼고 그 사실을 표시합니다. 통계 탭에는 전체 기간에 대한 같은 환산이 들어 있습니다.

토큰 종류마다 그 몫의 금액이 옆에 붙어 있는데, 놀라운 지점은 대개 여기입니다. 긴 대화에서는 **캐시 읽기가 비용의 대부분**을 차지합니다. 요청할 때마다 그때까지의 대화 전체를 다시 읽기 때문에, 세션 비용은 길이의 제곱으로 늘어납니다. 아래 24시간 분석이 긴 대화를 따로 짚어주는 것도 같은 이유입니다.
