...
- **단계별로 묶어서 보여줌** — 워크플로우가 단계를 둘 이상 선언했으면 에이전트를 그 단계 아래로 묶고, 묶음마다 완료/전체 숫자를 달아 보여줍니다. 단계 제목을 누르면 그 묶음이 접히고, 실행 중인 에이전트가 없는 단계는 처음부터 접혀 있습니다. 접어 둔 상태는 패널을 닫았다 열어도 기억합니다
- **에이전트별 모델과 토큰** — 줄마다 어떤 모델이 돌았고 토큰을 얼마나 썼는지 표시되며, 카드 머리에는 에이전트 수와 워크플로우 합계가 들어갑니다
