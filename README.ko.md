...
**Claude Code와 OpenAI Codex를 VS Code 상태바에 나란히** — 세션별 컨텍스트 사용량, 모델·Effort, 작업 완료 비프, 그리고 계정 한도(Claude.ai 5시간 세션 & 주간, Codex 5시간 & 주간 사용량)까지. 실시간 워크플로우/에이전트·백그라운드 작업 패널, 도는 작업을 알리는 상태바 점, Remote‑SSH 지원, 텔레그램 리셋 알림, 한/영 설정 패널 포함.
...
- **실행 중 점** — 워크플로우·백그라운드 작업·Codex 실행이 도는 동안 상태바 맨 앞에 색깔 점 표시 ([상태바의 실행 중 점](#-상태바의-실행-중-점) 참조)
...
상태바 표기는 Claude 쪽과 같습니다. 5시간 한도가 있는 계정은 5시간 수치와 카운트다운 뒤에 주간 수치가 붙고(`30% (2h 3m) - 49%`), 주간 한도만 있는 계정은 주간 수치를 카운트다운과 함께 보여 줍니다(`49% (2d 7h)`). 단독 **`⬢ Codex`** 아이템의 글자 색은 앞 수치를 따릅니다. 70%부터 노랑, 90%부터 빨강이고, 그 아래에서는 세션 아이템과 같은 평상시 글자색(`baseColor`)입니다. ⬢ 아이콘은 Codex 세션 아이템과 같은 하늘색을 유지합니다. 세션 아이템은 예전처럼 컨텍스트 사용률로 색이 정해집니다.
...
**읽기 방식의 차이 한 가지(성능 참고):** 로컬에서는 파일의 필요한 구간만(byte‑range) 읽어서 14.1MB짜리 rollout도 수 밀리초면 끝납니다. Remote‑SSH에서는 VS Code 파일 API에 구간 읽기가 없어 **파일 전체**를 읽습니다. 이는 Claude가 원격에서 이미 하고 있는 것과 동일한 방식입니다(이 개발 머신의 Claude 세션 파일 최대 크기는 9.2MB). 둘 다 **mtime과 크기가 그대로인 파일은 다시 읽지 않습니다** — Claude 대화 파일은 1.17.1부터입니다. 그전에는 갱신할 때마다 최근 `hideAfter`(기본 24시간) 안에 수정된 대화 파일을 전부 다시 읽어서, 창을 열어 둔 채 세션을 열고 닫을수록 갱신마다 옮기는 양이 늘었습니다. Remote‑SSH 창 하나에서 1분에 약 70~220MB가 측정됐습니다. 이제는 아직 쓰이고 있는 대화 파일만 다시 받습니다.

**세션 메뉴와 패널은 이런 읽기를 기다리지 않고 바로 뜹니다.** 1.17.1부터 확장이 이미 아는 값으로 곧바로 열고, 새 숫자는 도착하는 대로 그 자리에 채웁니다. 아직 못 읽은 건수는 `확인 중…` 으로, 첫 목록을 받기 전에 연 패널은 `불러오는 중…` 으로 보입니다. Remote‑SSH에서는 이 읽기가 상태바 갱신의 읽기 뒤에 줄을 서서, 창을 막 연 직후 메뉴가 뜨는 데 16~21초가 걸린 적이 있었습니다. 안전장치로, 원격에서 **32MB를 넘는** rollout 파일은 건너뛰고 로그에 남깁니다.
...
- **단계별로 묶어서 보여줌** — 워크플로우가 단계를 둘 이상 선언했으면 에이전트를 그 단계 아래로 묶고, Codex 패널의 턴 머리표 모양에 완료/전체 숫자를 달아 보여줍니다. 머리표를 누르면 접힙니다. 에이전트는 띄운 순서대로 나옵니다 — 폰의 원격 제어 화면과 같은 순서입니다
...
- **역할 라벨** — 워크플로우 대본이 지정한 `label` 을 그대로 쓰고, 없으면 프롬프트 헤더에서 추출합니다. "에이전트-1" 대신 의미 있는 이름으로 표시. 최근 Claude Code 는 에이전트를 띄울 때 그 라벨과 단계를 기록해 두는데, 패널은 이것부터 읽습니다. 그래서 대본 파일을 못 찾은 실행도 에이전트 이름과 단계가 그대로 나옵니다
- **Task(Agent 도구) 서브에이전트** — Agent 도구로 실행된 서브에이전트를 **시작 시각 기준 배치로 묶어** 카드로 따로 표시합니다 (5분 이상 간격이면 새 배치). 워크플로우와 똑같이 다룹니다. 끝난 배치의 🗑는 같은 휴지통으로 옮기고, 대화에 중단·강제 종료·실패로 알려진 서브에이전트는 중단으로 표시합니다. 그 알림 뒤에 기록이 더 늘었으면 다시 이어서 일하는 것이라 진행 중으로 둡니다
- **휴지통** — 끝난 워크플로우나 배치의 🗑를 누르면 없어지는 게 아니라 확인창 없이 옆으로 치워집니다. 패널 상단의 🗑를 열어 되살리거나 완전히 지우며, 프로젝트의 모든 세션 것이 한 목록에 나옵니다. 그 사이 같은 id의 워크플로우가 새로 생겼거나 같은 이름의 서브에이전트 기록이 다시 생겼으면 복구는 거부됩니다
...
세션 메뉴의 워크플로우 바로 아래에서 열거나, 명령 팔레트에서 `claudeStateBar: Show Claude Background Tasks` 로 엽니다. 상태바에 보이는 세션 — `hideAfter`(기본 24시간) 안에 활동한 세션 — 에 대해 두 가지를 보여줍니다. Claude 가 무엇을 백그라운드로 돌렸는지, 그리고 어떤 명령이 오래 걸렸는지입니다. 그래서 두 묶음으로 나뉩니다:

- **백그라운드** — Claude Code 가 `run_in_background` 로 돌린 명령과 띄운 모니터. 명령인지 모니터인지는 카드의 표식으로 구분합니다
- **오래 걸린 명령** — 2분 넘게 걸린 일반 명령. 확인한 컴퓨터들에서는 명령 250개 중 1개꼴이었습니다

묶음마다 머리에 실행 중·끝난 건수가 나오고, 머리를 누르면 접힙니다. 폰의 원격 제어 화면은 백그라운드 목록에 워크플로우를 섞어 보여주지만, 여기서는 워크플로우가 위의 자기 패널을 따로 쓰고, 워크플로우 에이전트가 띄운 명령도 뺍니다. Claude 가 `codex_rescue` 로 Codex 에 문제를 넘길 때 쓰는 백그라운드 명령도 뺍니다 — 그 실행은 Codex 진행 상황 패널에 나옵니다.

- **실행 중인 작업** — 카드마다 작업 설명, 실행한 지 얼마나 됐는지, 명령이 나옵니다. 백그라운드 작업은 지금까지의 출력도 나오는데, 출력 칸은 맨 아래에 두면 새 줄을 따라 내려가고, 패널이 열려 있는 동안 2초마다 다시 읽습니다. 일반 명령은 2분을 넘는 순간 나타나며, Claude Code 가 출력을 명령이 끝나야 기록하기 때문에 아직 보여줄 출력이 없습니다. 그래서 카드에 Claude 가 이 명령을 기다리는 중이라고 나옵니다
- **끝난 작업** — 묶음마다 끝난 것을 한 줄로 접어 묶으며, 최근 것부터 묶음당 10건까지 보여줍니다. 카드마다 완료·실패(종료 코드 포함)·중지 중 어떻게 끝났는지 나오고, 펼치면 명령과 출력이 보입니다. 돌아가는 동안 지켜본 작업은 패널을 닫을 때까지 묶음 밖에 남습니다. 그 줄의 🗑는 그 묶음의 끝난 작업을 목록에서 치웁니다 — 파일은 건드리지 않으며, 작업 영역(워크스페이스)마다 기억됩니다
- **끝났다고 보는 기준** — 백그라운드 작업은 대화에 종료 알림이 들어왔을 때, 또는 출력 파일이 이미 `[exited with code N]` 으로 끝났을 때입니다. 둘 다 없으면 "실행 중"으로 두므로, 작업보다 세션이 먼저 끝난 경우엔 그 세션이 상태바에서 빠질 때까지 실행 중으로 보일 수 있습니다. 일반 명령은 대화에 그 결과가 들어왔을 때입니다
- **지워진 출력** — Claude Code 는 백그라운드 작업의 출력을 임시 폴더에 두는데, 나중에 지워지는 경우가 많습니다. 그러면 카드에 그렇다고 나옵니다. 모니터는 그래도 알려 온 이벤트를 보여줍니다. 일반 명령의 출력은 대화 기록 안에 있어서 지워지지 않습니다
- **소리** — 백그라운드 작업이 완료되면 워크플로우 음이 같은 `workflowCompleteBeep` 설정으로 울립니다. 실패하거나 중지된 작업은 실패한 워크플로우처럼 조용합니다. 확장이 실행 중인 모습을 못 본 작업도 조용합니다 — 확장을 켜기 전에 이미 끝나 있던 작업이 그 예입니다. 오래 걸린 일반 명령은 소리를 내지 않습니다. Claude 가 그 명령을 기다리던 중이라, 끝나면 바로 이어서 답하기 때문입니다
- **보기 전용** — 중지 버튼은 없습니다. 작업의 프로세스가 디스크에 기록되지 않아 패널이 안전하게 멈출 방법이 없습니다. 멈추려면 채팅에서 Claude 에게 시키면 됩니다
- **Remote‑SSH** — 대화 기록은 상태바가 이미 읽는 파일을 같이 써서 추가 전송이 없고, 출력 파일은 작업을 돌린 쪽 컴퓨터에서 읽습니다

---

## 🟠 상태바의 실행 중 점

이 확장의 상태바 항목 맨 앞, 첫 세션 바로 왼쪽에 색깔 점이 최대 세 개 떠서 대화 뒤에서 무엇이 돌고 있는지 알려줍니다. 서브에이전트·백그라운드 명령·Codex 실행이 아직 도는지 패널을 열지 않아도 한눈에 보입니다.

- **주황** — 상태바에 보이는 세션에서 에이전트가 실행 중인 워크플로우가 있거나, Agent 도구 서브에이전트가 돌고 있을 때. 함께 띄운 서브에이전트는 워크플로우 패널처럼 한 묶음으로 셉니다
- **회색** — 그 세션들에서 백그라운드 작업(`run_in_background` 명령이나 모니터)이 돌고 있을 때. 오래 걸린 일반 명령은 켜지 않고, Claude 가 Codex 를 부르는 백그라운드 명령도 켜지 않습니다 — 그 실행은 파랑으로 보입니다
- **파랑** — [Codex 진행 상황 패널](#-codex-진행-상황-패널-선택)의 실행이 돌고 있을 때. 패널에 응답 없음으로 뜨는 실행은 켜지 않습니다 — 실행 도중 창을 다시 불러와 끊긴 경우가 그 예입니다
- **도는 동안만** — 점은 그 종류가 시작되면 나타나고 끝나면 사라집니다. 셋이 동시에 돌면 이 순서로, 상태바가 허용하는 만큼 바짝 붙어서 나옵니다
- **마우스 올리기·누르기** — 툴팁에 무엇이 도는지가 나옵니다. 워크플로우·서브에이전트 묶음마다 이름과 에이전트가 몇 개 끝났는지, 명령마다 설명과 시작 시각, Codex 실행마다 제목과 모드입니다. 점을 누르면 그 패널이 열립니다
- **반영 속도** — 주황·회색은 상태바 갱신(대화 기록이 바뀔 때, 또는 기본 30초마다)을 따르고, 파랑은 실행 중이면 2초마다 도는 Codex 확인을 따릅니다
- **색** — 점마다 테마 종류(어두운·밝은·고대비)별로 상태바에서 잘 보이는 색이 정해져 있습니다. VS Code 기본 테마 6종에서 세 점 모두 상태바 배경과 5:1 이상으로 대비됩니다. 바꾸려면 `workbench.colorCustomizations` 에 `claudeContextBar.workflowDot`·`claudeContextBar.backgroundDot`·`claudeContextBar.codexDot` 를 넣으면 됩니다. 예: `"workbench.colorCustomizations": { "claudeContextBar.workflowDot": "#FF8800" }`
...
- **같은 저장소의 작업 복사본에서 돈 실행도 보입니다** — 스킬은 자기가 돈 작업 트리 아래에 기록을 남기기 때문에, `git worktree add` 로 만든 작업 복사본에서 돈 실행은 원본 폴더를 연 창에 나오지 않았고 그 반대도 마찬가지였습니다. 이제 창에 열린 폴더가 속한 저장소의 작업 트리 전부에서 실행을 모아 보여 줍니다. 창에 원본을 열었든 복사본을 열었든 같습니다. 창의 폴더가 아닌 곳에서 돈 카드에는 `⎇ 폴더이름` 칩이 붙고, 마우스를 올리면 전체 경로가 나옵니다. Codex 채팅 패널도 똑같이 동작하고, 상태바의 Codex 점과 완료음도 이 실행들에 반응합니다
...
머리표에는 **그 턴이 시작한 시각과 걸린 시간**도 붙습니다. 카드 윗줄 시계는 1턴 시작부터 마지막 턴이 끝날 때까지를 재고, 옆에 Codex가 실제로 일한 시간을 함께 적습니다 — `소요 43:11 (작업 12:47)`. 둘의 차이는 턴 사이, Claude가 답을 읽고 다음 질문을 쓰던 시간입니다. 1.17.1 전에는 되물을 때마다 카드 시계가 새로 시작해서, 43분 걸린 6턴짜리 실행이 마지막 턴의 1분 20초만 보여 줬습니다. 2턴부터는 그 턴을 연 반박서가 저장된 시각을 턴 시작으로 읽기 때문에 몇 초 이르게 나올 수 있습니다. 몇 턴까지 갔는지는 모드 칩 옆에도 칩으로 붙어서(`FOLLOWUP` `6턴`) 카드를 접은 채로도 보입니다.
...
**단일 턴 실행의 화면은 그대로입니다.** 머리표도, 턴별 문서 링크·시각도, 접기도 되묻기가 실제로 있었던 카드에만 나타납니다.
...
## 🔍 browser_check — Claude가 실제 브라우저를 직접 확인 (플러그인)
...
| `claudeContextBar.workflowCompleteBeep` | `true` | Claude 워크플로우/Task 또는 Codex spawned-agent 전체 완료 시, 또는 백그라운드 작업 완료 시 워크플로 음 재생 |
