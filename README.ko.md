...
⚠️ **아직 스킬에 연결되지 않았습니다.** 배관은 이 저장소의 [`tools/live-consult/`](tools/live-consult/)에 있고 동작도 확인했지만(위 설명이 그 실측 결과입니다), `send.sh`는 여전히 `codex exec`를 부릅니다. 스킬을 평소대로 설치해도 끼어들기는 아직 쓸 수 없습니다. 연결은 다음 판이며, 남은 결정 사항까지 [통합 계획](docs/LIVE_STEER_INTEGRATION.md)에 적어 두었습니다.

**아래 턴 표시는 지금 바로 됩니다.** 스킬이 이미 쓰고 있는 기록만 읽기 때문입니다.
