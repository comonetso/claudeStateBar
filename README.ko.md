...
- **툴팁** — Codex 주간/보조 한도(사용 비율, 갱신 시각, 플랜 종류) + 컨텍스트 토큰 내역 + 세션 누적 처리량
...
- **사이드바 선택은 OpenAI 내부 로그 표식을 호환성 폴백으로 사용합니다.** Codex 에디터 탭은 안정적인 VS Code 탭 URI를 사용합니다. `active=false`는 단순 창 포커스 이탈 때도 발생하므로 가장 최근의 `active=true` UUID를 창별로 유지합니다. Remote‑SSH에서는 프로세스 ID로 로컬 창과 원격 OpenAI 확장 호스트 로그를 연결하고, 불가능할 때만 활성화 시각을 제한적 폴백으로 사용합니다. 향후 OpenAI 로그 형식이 바뀌면 계정 사용량 항목으로 안전하게 내려갑니다.
...
- **잘린 줄은 눌러서 봅니다** — 패널 폭을 넘어간 줄은 클릭하면 그 자리에서 워드랩으로 펼쳐집니다. 말은 전문이, 명령은 실제로 실행된 형태가 나옵니다. 한 번에 한 줄만 열립니다
...
- `.log/` — 원시 실행 기록. 명령 출력 전문이 담겨 실행당 300KB쯤 되고, 스킬이 이 디렉터리에 자체 `.gitignore`를 넣어 **git에서 제외**합니다
...
실행 기록은 **한 번에 약 300KB** 쌓이고, 그중 86% 정도가 명령 실행 결과 전문입니다.
켜지 않는 한 자동으로 지워지지 않습니다.
...
| `claudeContextBar.codexRunDeleteDocs` | `false` | **자동** 정리가 요청서·응답·리뷰 `.md` 문서까지 지울지. 기본은 꺼짐 — 그 문서는 무엇을 묻고 무엇을 답했는지의 기록이며 보통 커밋해서 남깁니다. 수동 삭제는 이 값과 무관하게 매번 물어봅니다 |

패널에서 🗑로 지울 때는 **문서까지 지울지 매번 물어보며**, 버튼은 끝난 실행에만 나타납니다.
