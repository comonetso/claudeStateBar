...
Reads Claude Code's session JSONL logs from `~/.claude/projects/` and displays real-time token usage in the VS Code status bar. No network calls, no SDK — pure disk reads.

### Core Data Flow

1. **`findActiveSessions()`** — scans `~/.claude/projects/`, filters by idle timeout, applies supersession logic, returns up to 5 sessions
...
**Path decoding (`decodeProjectPath`)**: Claude encodes project paths as directory names using dashes (e.g., `C--Users--foo--project`). The decoder reconstructs paths heuristically — uses the last 3 path segments as the display name. Windows paths (`C:\`) and Unix paths (`/`) are both handled.
...
**Model + effort display (1.5.0+)**: `getShortModelName()` maps `claude-sonnet-4-5-*` → `Sonnet 4.5` (or `S4.5` in compact mode). `getEffortIndicator()` returns `⚡` when `speed !== 'standard'` (Claude Code `/fast` toggle) and `💭` when any recent assistant message contains a non-empty `thinking` content block. JSONL has no explicit "effort" field — these are the closest available signals.
