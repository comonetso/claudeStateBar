...
그 밖의 명령(모두 `/peer-req:peer_req` 뒤에 붙입니다) — `here` 는 지금 대화를 이 저장소의 세션으로 지정합니다(같은 폴더에 창이 여러 개 떠 있을 때 유용합니다). `status` 는 최근 요청 목록, `inbox` 는 도착했거나 끝났는데 아직 안 알린 것, `doctor` 는 아무것도 보내지 않고 주소록과 설정을 점검합니다. `forget <짝>` 은 기억해 둔 짝 세션을 지웁니다 — 다음에 보낼 때 다시 찾습니다.
...
**다른 머신에서 알맞은 세션을 찾는 방식.** Remote Control 목록에는 세션 제목만 나오고, 어느 머신의 어느 폴더에서 도는 세션인지는 나오지 않습니다. 그래서 주소록에 `rc_title` 이 있으면 그 제목으로 찾고, 없으면 처음 한 번 목록에서 고르시게 합니다. 다른 짝이 이미 쓰는 세션을 빼고 다른 머신 세션이 하나뿐이면 묻지 않고 그리로 보냅니다. 고르신 세션은 이 PC 에 기억해 두고 다음부터 묻지 않습니다. Remote Control 제목은 같은 대화라면 VS Code 를 다시 로드하거나 대화를 나갔다 들어와도 그대로입니다. 이름을 붙여 제목을 바꾼 경우에도 목록의 참조 번호(`[87f895]` 같은 것)가 그대로면 같은 세션으로 알아봅니다. 새 대화를 열면 제목이 새로 생기므로 그때 한 번 다시 고르시면 됩니다. 엉뚱한 세션으로 가더라도 받는 쪽이 주소록으로 자기 앞으로 온 것인지 확인해 "대상 아님" 으로 돌려보냅니다. 그러면 그 세션은 기억에서 지우고 다음부터 후보에서 뺍니다.
