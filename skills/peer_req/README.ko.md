...
- **세션이 시작되면** 플러그인이 로드됐다는 것, 이 저장소의 짝이 몇 곳인지, 자리를 비운 사이 도착했거나 끝난 것이 있는지를 알려 줍니다. 세션에 제목이 없으면 "기기 · 프로젝트" 제목을 붙여 다른 머신의 목록에서 알아보기 쉽게 합니다. 직접 지은 제목은 건드리지 않고, 기존 제목을 확인할 수 없는 옛 Claude Code 에서는 붙이지 않습니다.
